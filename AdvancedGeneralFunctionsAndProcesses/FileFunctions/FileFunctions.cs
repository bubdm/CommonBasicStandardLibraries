...
namespace CommonBasicStandardLibraries.AdvancedGeneralFunctionsAndProcesses.FileFunctions
{
    public static class FileFunctions
    {
		...
		public static async Task FilterDirectoryAsync(CustomBasicList<string> firstList)
		{
			...
			await Task.Run(() =>
			{
				...
				{
					if (Directory.EnumerateFiles(items).Count() == 0)
        ...
        public static void FilterDirectory(CustomBasicList<string> firstList)
        {
            CustomBasicList<string> removeList = new CustomBasicList<string>();
            firstList.ForEach(items =>
            {
        ...
        public static CustomBasicList<string> GetDriveList()
        {
            DriveInfo[] completeDrives;
            CustomBasicList<string> newList = new CustomBasicList<string>();
            completeDrives = DriveInfo.GetDrives();
        ...
        public static CustomBasicList<string> DirectoryList(string whatPath, SearchOption searchOption = SearchOption.TopDirectoryOnly)
        {
            return Directory.EnumerateDirectories(whatPath, "*", searchOption).ToCustomBasicList();
        }
        public static CustomBasicList<string> FileList(string directoryPath, SearchOption thisOption = SearchOption.TopDirectoryOnly)
        {
            return Directory.EnumerateFiles(directoryPath, "*", thisOption).ToCustomBasicList();
        }
        public static CustomBasicList<string> FileList(CustomBasicList<string> directoryList) //this can be just top because you already sent in a directory list
        {
            CustomBasicList<string> newList = new CustomBasicList<string>();
        ...
        public static CustomBasicList<string> GetSeveralSpecificFiles(string directoryPath, string extensionEndsAt, SearchOption searchOption = SearchOption.TopDirectoryOnly) //to not break compatibility
        {
        ...
        /// <returns></returns>
        public static CustomBasicList<string> GetSeveralSpecificFiles(string directoryPath, string extensionEndsAt, CustomBasicList<string> goodList, SearchOption searchOption = SearchOption.AllDirectories)
        {
            //step 1 is getting the specific files without the good list
            CustomBasicList<string> firstList = GetSeveralSpecificFiles(directoryPath, extensionEndsAt, searchOption);
        ...
        public static async Task<CustomBasicList<string>> GetSeveralSpecificFilesAsync(string directoryPath, string extensionEndsAt, SearchOption searchOption = SearchOption.TopDirectoryOnly) //to not break compatibility
        {
        ...
        /// <param name="searchOption">default is all directories but can choose top if needed</param>
        /// <returns></returns>
        public static async Task<CustomBasicList<string>> GetSeveralSpecificFilesAsync(string directoryPath, string extensionEndsAt, CustomBasicList<string> goodList, SearchOption searchOption = SearchOption.AllDirectories)
        {
            //step 1 is getting the specific files without the good list
            CustomBasicList<string> firstList = await GetSeveralSpecificFilesAsync(directoryPath, extensionEndsAt, searchOption);
            firstList.KeepConditionalItems(items =>
            {
                string thisName = FileName(items).ToLower();
                if (goodList.Exists(Temps => Temps.ToLower() == thisName) == true)
        ...
        {
            //this time i need first a list of all but even sub folders for it.
            CustomBasicList<string> tFileList;
            tFileList = Directory.EnumerateFiles(directoryPath, fileName, SearchOption.AllDirectories).ToCustomBasicList();
            if (tFileList.Count == 0)
                throw new BasicBlankException($"The File Name {fileName} Was Not Found At {directoryPath} Or Even Sub Directories");
            return tFileList.Single(); //if nothing is found, then will get runtime error
        ...
        {
            CustomBasicList<string> thisList = FileList(directoryPath);
            thisList.KeepConditionalItems(Items =>
            Items.ToLower().EndsWith(stringEnd.ToLower()));
