    ...
    public abstract partial class JsonReader : IDisposable
    {
            ...
            switch (t)
            {
                ...
                case JsonToken.Float:
...
#endif
                    {
                        try
                        {
                            i = System.Convert.ToInt32(v, CultureInfo.InvariantCulture);
                        }
                        catch (Exception ex)
                        {
        ...
        private bool ReadArrayElementIntoByteArrayReportDone(List<byte> buffer)
        {
            switch (TokenType)
            {
                ...
                case JsonToken.Integer:
                    buffer.Add(System.Convert.ToByte(Value, CultureInfo.InvariantCulture));
                    return false;
                case JsonToken.EndArray:
                    return true;
            ...
            switch (t)
            {
                ...
                case JsonToken.Float:
...
#endif
                    {
                        try
                        {
                            d = System.Convert.ToDecimal(v, CultureInfo.InvariantCulture);
                        }
                        catch (Exception ex)
                        {
                            // handle error for large integer overflow exceptions
